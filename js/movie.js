...
function loadDisqus() {
  let url =
    $(".btn-comment-tab.active").data("type") === "anime"
      ? movie.shortlink
      : episode_play.shortlink;
    ...
    if (res.status) {
      ...
      if (currentSource && $(".servers-" + currentSource).length > 0) {
        ...
        if (currentServer && svEl.length > 0) {
          ...
          $(".servers-" + currentSource + " .server-item")
            .first()
            .trigger("click");
        }
      } else {
        $(".servers-mixed .server-item").first().trigger("click");
      }
    }
  });
}
  ...
  if (page == "movie_watch") {
    ...
    $(document).on("click", ".btn-vote", function () {
      ...
      if (typeof grecaptcha !== "undefined") {
        grecaptcha
          .execute(recaptchaSiteKey, { action: "vote" })
